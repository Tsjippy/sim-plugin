...
function request_month(target, month, year){
    ...
    if(calendar_page == null){
        target.closest('.calendar-wrap').insertAdjacentHTML('beforeEnd','<img class="loader" src="'+simnigeria.loading_gif+'" style="margin-left: auto;margin-right: auto;display: block;">');
...
function request_week(target, wknr, year){
    url.searchParams.set('yr', year);
...
function request_expand_list(offset, month, year){
    ...
    document.getElementById('listview').insertAdjacentHTML('beforeEnd','<img class="loader" src="'+simnigeria.loading_gif+'" style="margin-left: auto;margin-right: auto;display: block;">');
