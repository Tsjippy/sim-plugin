...
window['after_form_element_submit'] = function(result,responsdata){
	...
	if (result.status >= 200 && result.status < 400) {
		...
		//insert the node
		referenceNode = document.querySelector('form#simnigeria_form_'+responsdata.formid+' .form_elements [data-id="'+(parseInt(responsdata.index)-1)+'"]');
		if(referenceNode == null){
			var form_div = document.querySelector('form#simnigeria_form_'+responsdata.formid+' .form_elements');
			form_div.insertAdjacentElement('beforeend',node);
		}else{
			referenceNode.parentNode.insertBefore(node, referenceNode.nextSibling);
		}
		
		fix_element_numbering(document.querySelector('form#simnigeria_form_'+responsdata.formid));
...
window['prefillforminput'] = function(result,responsdata){
	if (result.status >= 200 && result.status < 400) {
		...
		modal.querySelector('[name="update"]').value = responsdata.element_id;
		...
		var loader = document.querySelector('form#simnigeria_form_'+responsdata.formid+' .form_element_wrapper[data-id="'+responsdata.element_id+'"] .loadergif');
		var template		= document.createElement('template');
...
window['removerow'] = function (result,responsdata){
	if (result.status >= 200 && result.status < 400) {
		...
		document.querySelector('form#simnigeria_form_'+responsdata.formid+" .form_elements").querySelector('[data-id="'+responsdata.elementindex+'"]').remove();
...
window['fixindexes'] = function(result,responsdata){
	fix_element_numbering(document.querySelector('form#simnigeria_form_'+responsdata.formid));
...
function clearforminputs(){
	try {
		...
		modal.querySelectorAll('input:not([type=hidden]), select, textarea, [name=insertafter], [name=update]').forEach(function(el){
			if(el.type == 'checkbox'){
				el.checked = false;
			}else{
				el.value = '';
...
function send_element_size(el,widthpercentage){
	if(widthpercentage != el.dataset.widthpercentage && Math.abs(widthpercentage - el.dataset.widthpercentage)>5){
		...
		formdata.append('elementindex',el.closest('.form_element_wrapper').dataset.id);
		formdata.append('formfield[width]',widthpercentage);
		formdata.append('remove_form_element_nonce',remove_form_element_nonce);
		formdata.append('formname',el.closest('.form_elements').querySelector('[name="formname"]').value);
		sendAJAX(formdata);
	}
}
...
function reorderformelements(event){
	if(reordering_busy == false){
		...
		formdata.append('elementindex',event.item.dataset.id);
		formdata.append('newIndex',(parseInt(event.item.dataset.id)+difference));
		formdata.append('remove_form_element_nonce',remove_form_element_nonce);
		formdata.append('formname',event.item.closest('.form_elements').querySelector('[name="formname"]').value);
		sendAJAX(formdata);
	}else{
		Swal.fire({
			icon: 'error',
...
function show_empty_modal(target){
	...
	if(form_element_wrapper != null){
		modal.querySelector('[name="insertafter"]').value = form_element_wrapper.dataset.id;
	}
...
function request_edit_element_data(target){
	var elementindex				= form_element_wrapper.dataset.id;
	var request_form_element_nonce	= document.querySelector('[name="request_form_element_nonce"]').value;
	var formname					= target.closest('.form_element_wrapper').dataset.formname;
	modal.querySelector('[name="update"]').value = form_element_wrapper.dataset.id;
...
function fix_element_numbering(form){
	var elemenrows	= form.querySelectorAll('.form_element_wrapper');
	for (let i = 0; i < elemenrows.length; i++) {
		elemenrows[i].dataset.id = i;
	}
}
...
window.addEventListener("click", event => {
	...
	if (target.closest('.elementtype') != null){
		...
		if(target.dataset.value == 'label'){
			form.querySelector('[name="formfield[wrap]"]').checked	= true;
		}
	}
