...
;// CONCATENATED MODULE: ./node_modules/@web-auth/webauthn-helper/src/useLogin.js
...
/* harmony default export */ const src_useLogin = (useLogin);
...
const request_webauthn = src_useLogin(
	{
		actionUrl: simnigeria.base_url+'/wp-json/simnigeria/v1/auth_finish',
		optionsUrl: simnigeria.base_url+'/wp-json/simnigeria/v1/auth_start',
...
async function request_email_code(){
	//add new one
	var loader				= "<img id='loader' src='"+simnigeria.loading_gif+"' style='height:30px;margin-top:-6px;float:right;'>";
	...
	formData.append('username',username);
 	...
 	var response = await fetch(simnigeria.ajax_url, {
		method: 'POST',
		credentials: 'same-origin',
		body: formData
	});
...
function logout_user(){
	...
	if(typeof(Swal) != 'undefined'){
		var options = {
			iconHtml: '<img src="'+simnigeria.loading_gif+'">',
			title: 'Logging out...',
	...
	.then(result=>{
		...
		location.href	= simnigeria.base_url;
	});
};
...
document.addEventListener("click", function(event){
	...
	if(target.id == "login_button"){
		...
		open_login_modal();
	}else if(target.id == "logout" || target.parentNode.id == "logout"){
		logout_user();
