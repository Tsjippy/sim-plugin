...
add_filter('sim_mandatory_audience_param', function($audienceHtml, $audience){
    ...
    $audienceHtml	.= "<label>";
        ...
        $audienceHtml	.= "<input type='checkbox' name='audience[nolocal]' value='everyone' $checked>";
...
add_action('sim_mandatory_save_audience_param', function($audiences, $post){
    if(isset($audiences['nolocal'])){
        ...
        foreach($users as $user){
            ...
            if(isset($visaInfo['permit_type']) and ($visaInfo['permit_type'][0] == 'no' or $visaInfo['permit_type'][0] == 'visa')){
