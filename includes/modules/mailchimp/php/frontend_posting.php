...
add_action('sim_after_post_save', function($post){
	...
	if(is_numeric($_POST['mailchimp_segment_id'])){
		...
		if($post->post_status == 'publish'){
            ...
            update_post_meta($post->ID,'mailchimp_extra_message', $extraMessage);
		}
	}
});

add_action('frontend_post_after_content', function($frontendcontend){
    $mailchimpSegmentId	    = get_post_meta($frontendcontend->post_id, 'mailchimp_segment_id', true);
    $mailchimpEmail		    = get_post_meta($frontendcontend->post_id, 'mailchimp_email', true);
    $mailchimpExtraMessage  = get_post_meta($frontendcontend->post_id, 'mailchimp_extra_message', true);
