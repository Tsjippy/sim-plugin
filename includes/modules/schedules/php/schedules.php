...
class Schedule{
	function __construct(){
		...
		$this->table_name		= $wpdb->prefix . 'simnigeria_schedules';
		...
		<div name='add_host' class="modal hidden">
			<div class="modal-content">
				...
				<form action="" method="post">
					...
					<?php
					echo user_select($text='',$only_adults=true,$families=true,$class='',$id='host');
					echo add_save_button('add_host','Add host','update_schedule'); 
					?>
				</form>
			</div>
		</div>
		...
		<div name='add_recipe_keyword' class="modal hidden">
			<div class="modal-content">
				...
				<form action="" method="post">
					...
					<?php
					echo add_save_button('add_recipe_keyword','Add recipe keywords','update_schedule'); 
					?>
				</form>
			</div>
		</div>
			...
			<div class="modal-content">
				...
				<form action="" method="post">
					...
					if($this->admin	==	true){
						...
						<?php
						echo user_select($text='',$only_adults=true,$families=false,$class='',$id='host');
					}
					
					echo add_save_button('add_timeslot','Add time slot','update_event add_schedule_row');
					?>
				</form>
			</div>
		</div>
		...
		if($this->admin == true){
			...
			<form>
				...
				<datalist id="website_users">
					<?php
					foreach(get_missionary_accounts(true) as $user){
				...
				<?php
				echo add_save_button('add_schedule', 'Add schedule');
			echo '</form>';
		}
		return ob_get_clean();
	}
	...
	function showschedules(){
		//js
		wp_enqueue_script('simnigeria_schedule_script');
		...
		foreach($this->schedules as $key=>$schedule){
			...
			if(($schedule->target == $this->user->ID or $schedule->target == has_partner($this->user->ID)) and !$this->admin and !$schedule->published) continue;
				...
				<div class="modal publish_schedule hidden">
					...
					<div class="modal-content">
						...
						<form action="" method="post" id="publish_schedule_form">
							...
							echo user_select($text='',$only_adults=true,$families=true,$class='',$id='select_schedule_target',$args);
							echo add_save_button('publish_schedule','Publish this schedule'); 
							?>
						</form>
					</div>
				</div>
	...
	function write_meal_cell($schedule, $date, $starttime){
		...
		if($event != null){
			...
			$class .= ' selected';
			$partner_id = has_partner($this->user->ID);
	...
	function add_schedule_events($title, $schedule, $add_host_partner=true, $add_partner=true){
		...
		if($add_host_partner){
			$host_partner	= has_partner($this->host_id);
		...
		if($add_partner){
			$partner_id	= has_partner($schedule->target);
		}else{
			$partner_id	= false;
		}
	...
	function add_schedule(){
		verify_nonce('add_schedule_nonce');
	...
	function remove_schedule(){
		...
		verify_nonce('remove_schedule_nonce');
		...
		if(is_numeric($schedule->target)){
			update_family_meta($schedule->target,'schedule','delete');
		}
	...
	function add_host(){
		verify_nonce('update_schedule_nonce');
		...
		$partner_id		= has_partner($this->host_id);
	...
	function publish_schedule(){
		...
		verify_nonce('publish_schedule_nonce');
		...
		$schedule	= $this->find_schedule_by_id($schedule_id);
		update_family_meta($_POST['select_schedule_target'],'schedule',$_POST['schedule_index']);
