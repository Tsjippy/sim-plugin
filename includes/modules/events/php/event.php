				...
				<div class="eventlist">
					...
					foreach($this->events as $event){
						...
						if(is_numeric($user_id)){
							...
							$event_url	= get_user_page_link($user_id);
						}else{
...
function event_specific_fields($post_type,$post_id){
	...
	<div class="event <?php if($post_type != 'event') echo 'hidden'; ?>">
		...
		<label name="organizer">
			...
			<datalist id="users">
				<?php
				foreach(get_user_accounts(false,true,true) as $user){
		...
		if(is_array($meta['repeat'])){
			...
			if($freq == 'CUSTOM_DAYS'){
					...
					if(is_array($weeks)){
						...
						foreach($weeks as $index=>$week){
							...
							if($freq == 'MONTHLY'){
								...
								$ical_event	.= number_to_words($week);
							}
				...
				if(!empty($meta['repeat']['excludedates'])){
					...
					clean_up_nested_array($excludedates);
					if(!empty($excludedates)){
						$ical_event	.="EXDATE:";
