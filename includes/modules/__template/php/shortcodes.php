...
add_action( 'enqueue_scripts', function ($hook) {
	//Only load on sim settings pages
	if(strpos($hook, 'sim-settings') === false) return;

	wp_register_style('sim_admin_css', plugins_url('css/admin.min.css', __DIR__), array(), ModuleVersion);
	wp_register_script('sim_admin_js', plugins_url('js/admin.js', __DIR__), array('niceselect') ,ModuleVersion, true);
});