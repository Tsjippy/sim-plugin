...
add_action('frontend_post_save_meta', 'SIM\store_recipe_meta',10,2);
...
function store_recipe_meta($post_id, $post_type){
	//store recipetype
	$recipetypes = [];
	if(is_array($_POST['recipetype'])){
		...
		//Store recipe type
		wp_set_post_terms($post_id,$recipetypes,'recipetype');
	}
	...
	if(isset($_POST['time_needed']) and is_numeric($_POST['time_needed'])){
		//Store time_needed
		update_post_meta($post_id,'time_needed',$_POST['time_needed']);
	}
