...
class FormTable extends Formbuilder{
	...
	function transform_inputdata($string,$field_name){
		...
		if(is_array($string)){
			...
			$output = apply_filters('transform_formtable_data',$output,$field_name);
			return $output;
		}
	}
	...
	function add_shortcode_settings_modal(){
		...
		<div class="modal form_shortcode_settings hidden">
			...
			<div class="modal-content" style='max-width:90%;'>
				...
				<button class="button tablink <?php echo $active1;?>" data-target="column_settings_<?php echo $this->shortcodedata->id;?>">Column settings</button>
				<button class="button tablink <?php echo $active2;?>" data-target="table_rights_<?php echo $this->shortcodedata->id;?>">Table settings</button>
				...
				<div class="tabcontent <?php echo $class1;?>" id="column_settings_<?php echo $this->shortcodedata->id;?>">
					<form class="sortable_column_settings_rows">
						...
						foreach ($this->column_settings as $element_index=>$column_setting){
							...
							$icon			= "<img class='visibilityicon $visibility' src='".plugins_url()."/custom-simnigeria-plugin/includes/pictures/$visibility.png'>";
						...
						?>
						<?php
						echo add_save_button('submit_column_setting','Save table column settings');
						?>
					</form>
				</div>
				...
				<div class="tabcontent <?php echo $class2;?>" id="table_rights_<?php echo $this->shortcodedata->id;?>">
					...
					<?php
					echo add_save_button('submit_table_setting','Save table access settings');
					?>
					</form>
				</div>
			</div>
		...
		<div class='form_table_wrapper'>
			...
			if(count($this->submission_data) != 0){
				...
				*/
				...
				$this->owndata	= false;
				$this->user->partner_id		= has_partner($this->user->ID);
