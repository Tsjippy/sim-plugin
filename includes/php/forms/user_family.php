		...
		if ($user->ID != $user_id){
			...
			*/
			...
			//this user does not have a spouse         
				...
				if ($current_user_gender != "" or $gender != "" or $current_user_gender != $gender){
					//hide this user as potential spouse as it is a part of the family
					if (!in_array($user->ID, $family) and !in_array($user->ID,$children)){
						if($current_user_age == null or $current_user_age > 18){
...
add_filter('before_saving_formdata',function($formresults, $formname, $user_id){
	...
	if($family != $old_family){
		...
		create_missionary_page($user_id);
	}
