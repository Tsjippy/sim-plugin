...
function show_prayer(){
	...
	if(is_page($LoggedInHomePage)){
		...
		//Birthday, only display if there is a birthday
...
function blog_content() {
	...
	if(is_page($LoggedInHomePage) or is_front_page()){
		...
		while ( $loop->have_posts() ){
			...
			$expirydate = get_post_meta(get_the_ID(),'expirydate',true);
				
			//exclude if expired
			if($expirydate != '' and date('Y-m-d')>$expirydate) continue;
			
			if(get_post_type() == 'event' and get_post_meta(get_the_ID(), 'onlyfor', true)){
				continue;
			}
			?>
			<article class="news-article">
